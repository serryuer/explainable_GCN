...
class LabelGCNNet(torch.nn.Module):
    def __init__(self, 
                 node_size, 
                 embed_dim,
                 ...
                 dropout = 0.5,
                 layers = 2):
        super(LabelGCNNet, self).__init__()
    ...
    def forward(self, x, edge_index, edge_attr, train_mask):
        ...
        cosine_similarity_matrix = [torch.cosine_similarity(x[-i].unsqueeze(0), x[train_mask]) for i in range(1, self.num_class+1)]
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = F.relu(conv(x, edge_index, edge_weight=edge_attr))
        return F.log_softmax(x, dim=1), cosine_similarity_matrix
