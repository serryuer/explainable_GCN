...
parser.add_argument('-node_size', type=int, default=29428,
...
data = pkl.load(open(
    f"../{args.dataset_name}/graph/ind.{args.dataset_name}_id_hetero_label", 'rb'), encoding='latin1')
...
def train():
    model.train()
    optimizer.zero_grad()
    preds = model(data.x, [data.w2w_edge_index, data.w2d_edge_index, data.d2w_edge_index, data.d2l_edge_index, data.l2d_edge_index])
...
def test():
    ...
    logits = model(data.x, [data.w2w_edge_index, data.w2d_edge_index, data.d2w_edge_index, data.d2l_edge_index, data.l2d_edge_index])
